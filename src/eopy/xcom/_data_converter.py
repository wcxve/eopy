...
def read_data_for_element(path_to_xcom_data, Z) -> XCOMData:
    ...
    result = XCOMData()
    with open(
        os.path.join(path_to_xcom_data, SOURCE + '.' + str(Z).rjust(3, '0'))
    ) as fin:
        file = []
    ...
    file_start = 4
    if result.MAXEDG > 0:
        result.IDG = np.fromiter(
            map(int, file[4 : 4 + result.MAXEDG]), dtype='i'
        )[::-1]
        result.ADG = file[
            4 + result.MAXEDG : 4 + result.MAXEDG + result.MAXEDG
        ][::-1]
        result.EDGEN = np.fromiter(
            map(
                float,
                file[
                    4 + result.MAXEDG + result.MAXEDG : 4
                    + result.MAXEDG
                    + result.MAXEDG
                    + result.MAXEDG
                ],
    ...
    result.E = np.fromiter(
        map(float, file[file_start : file_start + result.MAXE]),
        dtype=[(NameProcess.ENERGY, 'd')],
    )
    file_start += result.MAXE
    result.SCATCO = np.fromiter(
        map(float, file[file_start : file_start + result.MAXE]),
        dtype=[(NameProcess.COHERENT, 'd')],
    )
    file_start += result.MAXE
    result.SCATIN = np.fromiter(
        map(float, file[file_start : file_start + result.MAXE]),
        dtype=[(NameProcess.INCOHERENT, 'd')],
    )
    file_start += result.MAXE
    result.PHOT = np.fromiter(
        map(float, file[file_start : file_start + result.MAXE]),
        dtype=[(NameProcess.PHOTOELECTRIC, 'd')],
    )
    file_start += result.MAXE
    result.PAIRAT = np.fromiter(
        map(float, file[file_start : file_start + result.MAXE]),
        dtype=[(NameProcess.PAIR_ATOM, 'd')],
    )
    file_start += result.MAXE
    result.PAIREL = np.fromiter(
        map(float, file[file_start : file_start + result.MAXE]),
        dtype=[(NameProcess.PAIR_ELECTRON, 'd')],
    )
    file_start += result.MAXE
    result.MAXK = 0
    ...
    if result.MAXEDG > 0:
        ...
        file_start += 1
        KMX = np.fromiter(
            map(int, file[file_start : file_start + LAX]), dtype='i'
        )
        file_start += LAX
        result.ENG = []
        for L in range(LAX):
            result.ENG.append(
                list(map(float, file[file_start : file_start + KMX[L]]))
            )
            file_start += KMX[L]
        result.PHC = []
        for L in range(LAX):
...
def convertMDATX3toHDF5(path_to_xcom_data):
    ...
    with tables.open_file(filename, 'w') as h5file:
        ...
        for Z in range(1, 101):
            ...
            temp_names = [
                name for name in map(lambda x: x.dtype.names[0], temp_tuple)
            ]
            temp_data = [
                it[name]
                for name, it in zip(temp_names, temp_tuple, strict=False)
            ]
            data_to_file = np.core.records.fromarrays(
                temp_data, names=','.join(temp_names)
            )

            delta = np.diff(data_to_file[NameProcess.ENERGY])
            if np.any(delta <= 0):
                print(f'Drop doubling for {data.IZ}')
                data_to_file = drop_doubling(data_to_file)

            table = h5file.create_table(
                group, 'data', obj=data_to_file, filters=filters
            )
            ...
            if MAXEDG > 0:
                ...
                data_to_file = np.core.records.fromarrays(
                    [data.IDG, np.array(data.ADG, dtype='S'), data.EDGEN],
                    names=','.join(name),
                )
                table = h5file.create_table(
                    edge_group, 'info', obj=data_to_file, filters=filters
                )
                table.attrs['energy_unit'] = 'eV'
                table.flush()
                for e, p, name in zip(
                    data.ENG, data.PHC, data.ADG, strict=False
                ):
                    data_to_file = np.core.records.fromarrays(
                        [e, p], names='energy,photoelectric'
                    )
                    table = h5file.create_table(
                        edge_group, name, obj=data_to_file, filters=filters
                    )
